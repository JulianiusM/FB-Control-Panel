...
module.exports = class SpeedtestExecutor {
    constructor(clients) {
        ...
        setTimeout(this.executeSpeedtest.bind(this), 5000);
    }
    ...
    executeSpeedtest() {
        ...
        if (this.settings.isRemoteSpeedtest()) {
            let protocolls = this.settings.getRemoteSpeedtestUrl().split("://", 2);
            let endpoints = protocolls[1].split("/", 2);

            this.restExecutor.performJSONRequest(protocolls[0], endpoints[0], "/" + endpoints[1], "GET", "", function (result) {
                if (result != undefined && result.empty != true) {
                    self.cachedResult = result;
                }
                setTimeout(self.executeSpeedtest.bind(self), 5 * 60000);
            });
        } else {
            (async () => {
                try {
